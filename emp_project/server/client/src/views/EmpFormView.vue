...
export default {
    data() {
        return {
            isUpdated : false, // 등록 기본 동작 => 수정 mode 
            searchNo: '',
            empInfo: {
                emp_no : '',
                first_name : '',
    ...
    created() {
        ...
        if(this.searchNo > 0){
            this.getEmpInfo();
            this.isUpdated = true; // true => 급여, 소속부서 선택 막음
        }else{
            this.empInfo.hire_date = this.getToday(); // 날짜 고정값 display 하려고(사용자편의성높이려고)
        }
    ...
    methods: {
        async getEmpInfo() {
            ...
            this.empInfo.hire_date = this.$dateFormat(this.empInfo.hire_date, 'yyyy-MM-dd'); //받고 한번더 변환하는 작업.
        },
        ...
        getInfo(eno){
            ...
            if(eno > 0 ){
                ...
                info.from_date = this.empInfo.hire_date; //필드하나 추가한거.- deptemp, salaries : fromdate필요함.
                data = {
                    param : this.empInfo
                };
            }
        ...
        async empInsert(){
          let info = this.empInfo;  
          info.from_date = this.empInfo.hire_date; //추가되야하는거! 등록할때.
          let data = {
                  param : this.empInfo
              };
